...
# 큰 단위가 작은 단위의 배수 형태이므로, 큰 화폐부터 작은 화폐까지 차례대로 확인하여 거슬러 줄 수 있다
# 하지만 배수 형태가 아닐 경우, 다른 정답을 찾아야 한다.(그리디알고리즘으로는 부족, 다이나믹 프로그래밍으로 해결)
# ex) 500 400 100 (잔돈 : 800) 500 + 100 * 3 보다 400 * 2가 최적의 답이 된다.
